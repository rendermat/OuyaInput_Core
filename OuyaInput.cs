...
public enum OuyaControllerType {Android, Broadcom, GameStick, MogaPro, Ouya, PS3, XBox360, TattieBogle, Unknown, None}
...
public static class OuyaInput
{
	...
	private enum EditorWorkPlatform {MacOS, Windows}
	...
	static OuyaInput() {
		...
		case RuntimePlatform.OSXEditor: editorWorkPlatform = EditorWorkPlatform.MacOS; break;
		case RuntimePlatform.WindowsEditor: editorWorkPlatform = EditorWorkPlatform.Windows; break;
		}
...
#endif
			case OuyaControllerType.Android:
				// remap values from range -1to1 onto range 0to1
				switch (triggerAxis) {
				case OuyaAxis.LT:
					// we only retrun a converted value if the trigger was initialized
	...
	private static OuyaControllerType findControllerType(int index, string controllerName) {
		...
		case "BT HID":
			controllerTypes[index] = OuyaControllerType.Android;
			return OuyaControllerType.Android;
		...
		case "":
			// on MacOSX the TattieBogle driver sends a blank "" name
			// this is not absolutely reliable but we have to other way to cover Tattie
			// unfortunately we have to neglect other blank controllers on MacOSX
			...
			// on other platforms the "" name is an unknown controller
			else {
...
#if !UNITY_EDITOR && UNITY_ANDROID
			case OuyaControllerType.Android:
				// tested on the HTC One using standard Bluetooth connection
				switch (button)
				{
				// ouya buttons
                case OuyaButton.O:		return GetButton(0, buttonAction, player);
                case OuyaButton.U:		return GetButton(1, buttonAction, player);
                case OuyaButton.Y:		return GetButton(2, buttonAction, player);
                case OuyaButton.A:		return GetButton(3, buttonAction, player);
					
				// shoulder buttons
                case OuyaButton.LB:		return GetButton(4, buttonAction, player);
                case OuyaButton.RB:		return GetButton(5, buttonAction, player);
					
				// stick buttons
                case OuyaButton.L3:		return GetButton(6, buttonAction, player);
                case OuyaButton.R3:		return GetButton(7, buttonAction, player);
					
				// d-pad buttons
                case OuyaButton.DU:		return GetButton(8, buttonAction, player);
                case OuyaButton.DD:		return GetButton(9, buttonAction, player);
                case OuyaButton.DL:		return GetButton(10, buttonAction, player);
                case OuyaButton.DR:		return GetButton(11, buttonAction, player);
